...
{
    //#NT#2010/10/26#Marx Chiu -begin
    //Fix empty folder issue.
    UH      FileFmt = FST_FMT_JPG;
    INT     ErrorStatus = FST_STA_ERROR;
    FST_DCF_INFO *pDCFInfo;
    //#NT#2010/10/26#Marx Chiu -end
    ...
    UIPrintPlayObj.uiMemorySize             = POOL_SIZE_IQ + POOL_SIZE_CAPTURE - POOL_SIZE_WAV;
    UIPrintPlayObj.CallBackFunc             = NULL;
    UIPrintPlayObj.Decode1stASFFunc         = NULL;
    ...
    UIPrintPlayObj.Flags                    = PB_DISABLE_TRANSFROM;
    ...
    UIPrintPlayObj.uiPanelWidth             = PB_PANEL_NO_SCALING;
    UIPrintPlayObj.uiPanelHeight            = PB_PANEL_NO_SCALING;
    ...
    pDCFInfo = FilesysGetDCFInfo();
    UIPRINT_MSG(("UIPRINT_M:UIPrint_Playback_Open:MaxDirID=%d\r\n",pDCFInfo->MaxDirID));

    for(maxDirId=pDCFInfo->MaxDirID; maxDirId>=MIN_DCF_DIR_NUM; maxDirId--)
    {
        FilesysChangeWorkingDir(maxDirId);

        if(pDCFInfo->MaxFileID)
        {
            UIPRINT_MSG(("UIPRINT_M:UIPrint_Playback_Open:DirID=%d,MaxFileID=%d\r\n",maxDirId,pDCFInfo->MaxFileID));
            for(maxFileId=pDCFInfo->MaxFileID; maxFileId>=MIN_DCF_FILE_NUM; maxFileId--)
            {
                ErrorStatus = FilesysQueryID(
                                FST_CMD_QUERY_SPECI_ID,
                                &maxDirId,
                                &maxFileId,
                                &FileFmt,
                                TRUE,
                                0);
                UIPRINT_MSG(("UIPRINT_M:UIPrint_Playback_Open:DirId=%d,FileId=%d,ErrorStatus=%d\r\n",maxDirId,maxFileId,ErrorStatus));
                if(ErrorStatus == FST_STA_OK)
                {
                    break;
                }
            }
