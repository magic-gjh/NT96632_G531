...
UINT8* UserPS_InitMP3FontData(void)
{
    ...
    // Read mp3 font data from PStore to this memory pool
    pMemPool = (pMemPool + POOL_SIZE_CAPTURE - POOL_SIZE_MP3FONT);

    PStore_EnablePS();

    if((pSection = PStore_OpenSection(PS_MP3FONT_DATA, PS_RDONLY)) != E_PS_SECHDLER)
    {
        // Get mp3 font data size first (leading 4 bytes)
        if(PStore_ReadSection(pMemPool, 0, 4, pSection) != E_PS_OK)
        {
            debug_err(("UserPS: PStore read mp3 font data size fail\r\n"));
            return NULL;
        }

        uiReadSize = MAKE_LONG(MAKE_WORD(pMemPool[0], pMemPool[1]), MAKE_WORD(pMemPool[2], pMemPool[3]));
        debug_err(("UserPS: Gotta read mp3 font data size = %ld\r\n", uiReadSize));
        if(uiReadSize > POOL_SIZE_MP3FONT)
        {
        ...
        {
            debug_err(("UserPS: PStore read mp3 font data fail\r\n"));
            return NULL;
        }
...
UINT32 UserPS_WriteMP3FontFile(CHAR *pFilename)
{
    PPSTORE_SECTION_HANDLE  pSection;
    HNVT_FILE               *pFile;
    ...
    if(uiFileSize > POOL_SIZE_MP3FONT)
    {
    ...
    *(pMemPool+3) = HI_BYTE(HI_WORD(uiWriteSize));
    ...
    if((pSection = PStore_OpenSection(PS_MP3FONT_DATA, PS_RDWR | PS_CREATE)) != E_PS_SECHDLER)
    {
        if(PStore_WriteSection(pMemPool, 0, (uiWriteSize+4), pSection) != E_PS_OK)
        {
            debug_err(("UserPS: PStore write mp3 font data fail\r\n"));
        }
        PStore_CloseSection(pSection);
        //PStore_DisablePS();
    }
