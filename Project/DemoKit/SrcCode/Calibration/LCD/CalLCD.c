...
void Cal_LCDVerify(void)
{
    ...
    FB_Addr.B0_Y            = Cal_GetMemoryAddr();
    ...
    //#NT#2007/06/08# Shirley Cheng -begin
    FB_Addr.uiBufSel = IDE_VIDEO_ADDR_SEL_BUFFER0 | IDE_VIDEO_ADDR_SEL_BUFFER1;
    //#NT#2007/06/08# Shirley Cheng -end
    ...
    ide_disable_osd(IDE_OSDID_2);
    //ide_set_osd1_en(FALSE);
    //ide_set_osd2_en(FALSE);
    //#NT#2009/09/01#KS Hung -end
    ...
    while (1)
    {
        wai_flg((FLGPTN *)&uiFlag, FLG_ID_KEY, FLGKEY_ENTER | FLGKEY_MENU, TWF_ORW | TWF_CLR);
