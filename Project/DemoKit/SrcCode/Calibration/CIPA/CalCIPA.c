...
void Cal_CIPA(void)
{
    ...
    while (1)
    {
        wai_flg((FLGPTN *)&uiFlag, FLG_ID_KEY, FLGKEY_TIMER1S, TWF_ORW | TWF_CLR);
        ...
        Cal_ShowString(StringBuffer, NULL, TRUE);

        if (bZoomIn == TRUE)
        {
            if (Photo_OpticalZoom_GetSection() >= MAX_ZOOM_SECTION)
            {
                uiTotalPicNum++;
        ...
        {
            if (Photo_OpticalZoom_GetSection() <= MIN_ZOOM_SECTION)
            {
                uiTotalPicNum++;
                Cal_CIPAUpdatePicNum();
                bZoomOut = FALSE;
