...
static void BMP_ValidatePICT(BMPStruct *pgBMPStruct)
{
 UW   pos;
    ...
    //BMP_hwmemset((UINT32)&g_BMPHeader,0,sizeof(g_BMPHeader));
...
static void BMP_CreateOSDBitmap(BMPStruct *pgBMPStruct)
{
 ...
 UINT8  uiColorindex;
    ...
    ide_waitFrameEnd();
    ide_getOsdBufAddr(IDE_OSDID_1,&uiBufAddr);
...
static void BMP_CreateBMPFile(BMPStruct *pgBMPStruct)
{
 ...
 char   filename[Filename_Len];
    ...
    // get FileID to make one new file
    //if(bupdate == FALSE)
    //{
    //  FileID = pDCFInfo->FilenameID+10;
    //  bupdate = TRUE;
    //}

    // create new filename
    //sprintf(filename,"A:\\PIC%04d.bmp",FileID);
