...
void GPIOMap_Init(void)
{
    ...
    // REC key
    gpio_setDir(GPIO_KEY_REC, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_REC, PAD_PULLDOWN);

    // UP key
    gpio_setDir(GPIO_KEY_UP, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_UP, PAD_PULLDOWN);

    // DOWN key
    gpio_setDir(GPIO_KEY_DOWN, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_DOWN, PAD_PULLDOWN);

    // LEFT key
    gpio_setDir(GPIO_KEY_LEFT, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_LEFT, PAD_PULLDOWN);

    // RIGHT key
    gpio_setDir(GPIO_KEY_RIGHT, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_RIGHT, PAD_PULLDOWN);

    // ENTER key
    gpio_setDir(GPIO_KEY_ENTER, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_ENTER, PAD_PULLDOWN);

    // PLAYBACK key
    gpio_setDir(GPIO_KEY_PLAYBACK, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_KEY_PLAYBACK, PAD_PULLDOWN);
#if (_PCB_ == _PCB_DV566_)//NT 20100210 Sun Chen Add

    gpio_setDir(GPIO_LCD_ROTATE, GPIO_DIR_INPUT);
    pad_setPullUpDown(GPIO_LCD_ROTATE, PAD_PULLDOWN);

    gpio_setDir(GPIO_LIGHT_COMPENSATE, GPIO_DIR_OUTPUT);
    gpio_clearPin(GPIO_LIGHT_COMPENSATE);   
    ...
    */
    ...
    // LCD power (ON: LOW; SLEEP: HIGH) => LCD power control is useless in DV569
...
UINT32 GPIOMap_GetKey(void)
{
    ...
    else if (gpio_getPin(GPIO_KEY_REC) == 1)
    {
        return VOLDET_KEY_LVL_REC;
    }
    else if (gpio_getPin(GPIO_KEY_UP) == 1)
    {
        return VOLDET_KEY_LVL_UP;
    }
    else if (gpio_getPin(GPIO_KEY_DOWN) == 1)
    {
        return VOLDET_KEY_LVL_DOWN;
    }
    else if (gpio_getPin(GPIO_KEY_LEFT) == 1)
    {
        return VOLDET_KEY_LVL_LEFT;
    }
    else if (gpio_getPin(GPIO_KEY_RIGHT) == 1)
    {
        return VOLDET_KEY_LVL_RIGHT;
    }
    else if (gpio_getPin(GPIO_KEY_ENTER) == 1)
    {
        return VOLDET_KEY_LVL_ENTER;
    }
    else if (gpio_getPin(GPIO_KEY_PLAYBACK) == 1)
    {
        return VOLDET_KEY_LVL_PLAYBACK;
    }
}

UINT32 GPIOMap_GetSpecialKey(void)
{
    return ((gpio_getPin(GPIO_KEY_ENTER) == 1) && (gpio_getPin(GPIO_KEY_PLAYBACK) == 1));
}
...
UINT32 GPIOMap_DetKey(void)
{
    ...
    if (GPIOMap_GetKey() == VOLDET_KEY_LVL_SHUTTER2)
    {
        //uiKeyCode |= FLGKEY_SHUTTER2;
        uiKeyCode |= FLGKEY_MODE;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_REC)
    {
        //uiKeyCode |= FLGKEY_REC;
        uiKeyCode |= FLGKEY_MENU;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_UP)
    {
        uiKeyCode |= FLGKEY_UP;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_DOWN)
    {
        uiKeyCode |= FLGKEY_DOWN;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_LEFT)
    {
        uiKeyCode |= FLGKEY_LEFT;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_RIGHT)
    {
        uiKeyCode |= FLGKEY_RIGHT;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_ENTER)
    {
        uiKeyCode |= FLGKEY_ENTER;
    }
    else if (GPIOMap_GetKey() == VOLDET_KEY_LVL_PLAYBACK)
    {
...
void GPIOMap_TurnOnLED(UINT32 uiLED)
{
    ...
    //case GPIOMAP_LED_COMPENSATELIGHT:
    //    gpio_setPin(GPIO_LIGHT_COMPENSATE); 
    //    break; 
