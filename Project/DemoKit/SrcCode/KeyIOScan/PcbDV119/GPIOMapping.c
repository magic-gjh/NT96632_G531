...
//--------------------------------------------------------------------
    ...
    //pad_setPullUpDown(PAD_TV_DETECT, PAD_PULLDOWN);
...
UINT32 GPIOMap_DetKey(void)
{
    ...
    else if (uiADCValue == VOLDET_KEY_LVL_ENTER)
    {
...
{
    //debug_err(("[GPIOMap_DetTV]: %d\r\n", gpio_getPin(GPIO_TV_DETECT)));
...
{
    //debug_err(("[GPIOMap_DetHDMI]: %d\r\n", gpio_getPin(GPIO_HDMI_DETECT)));
