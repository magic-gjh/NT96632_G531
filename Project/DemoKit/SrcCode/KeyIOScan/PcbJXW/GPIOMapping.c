...
//--------------------------------------------------------------------
    ...
    pad_setPullUpDown(PAD_KEY_SHUTTER, PAD_PULLUP/*?PAD_PULLUP*/);
    ...
    debug_err(("#SN_PWDN pull down\r\n"));
    pad_setPullUpDown(PAD_CCD_PCTL, PAD_PULLDOWN/*?PAD_PULLUP*/);
    ...
    #if 0
    pad_setPullUpDown(PAD_PIN_DGPIO3, PAD_PULLUP/*?PAD_PULLUP*/);
    #endif
...
UINT32 GPIOMap_DetKey(void)
{
   ...
   else if(uiADCValue == VOLDET_KEY_LVL_MENU)
    {
    ...
    else if(KeyScan_CheckEngModeGpio()==0)
    {
        uiKeyCode |= FLGKEY_ENTER;
    }
    else
...
//#NT#2011/04/28#Isiah Chang -begin
    debug_err(("#Sensor power on reset\r\n"));
    gpio_clearPin(GPIO_CCD_PCTL);//enable sensor power
    TimerDelayMs(100);
//#NT#2011/04/28#Isiah Chang -end
}
