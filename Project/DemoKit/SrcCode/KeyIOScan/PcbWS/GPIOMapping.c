...
//--------------------------------------------------------------------
    // Sensor LDO power (2.8V & 1.5V)
    //Must turn-on MES1 first (for digital 2.8V)
    gpio_setDir(GPIO_MSHUT_2, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_MSHUT_2);
    //Then turn-on MES0 (for analog 2.8V & external 1.5V)
    ...
    // Macro detect, HIGH active
    gpio_setDir(GPIO_MACRO_DETECT, GPIO_DIR_INPUT);
    pad_setPullUpDown(PAD_MACRO_DETECT, PAD_PULLDOWN);
    ...
    // Battery detection control enable
    gpio_setDir(GPIO_BATT_DET_EN, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_BATT_DET_EN);
}
