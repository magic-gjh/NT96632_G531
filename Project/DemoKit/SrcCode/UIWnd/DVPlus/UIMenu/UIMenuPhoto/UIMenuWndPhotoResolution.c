...
{
    UxMenu_SetData(&UIMenuWndPhotoRes_MenuCtrl,MNU_CURITM,Get_SizeIndex());
...
{
    Ux_CloseWindow(&UIMenuWndPhotoResCtrl,0);
    return NVTEVT_CONSUME;
}
INT32 UIMenuWndPhotoRes_Menu_OnKeyEnter(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    Ux_SendEvent(&UIPhotoObjCtrl,NVTEVT_EXE_SIZE,1,value);
	Ux_CloseWindow(&UIMenuWndPhotoResCtrl,0);
    return NVTEVT_CONSUME;
}
