...
//
static ER virtualSdio2_openMemBus(void);
static ER virtualSdio2_closeMemBus(void);
static ER virtualSdio2_writeSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt);
static ER virtualSdio2_readSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt);
static ER virtualSdio2_format(void);
...
static STRG_TAB gSDIOObj = {STORAGE_DETACH|STORAGE_SDIO, &gSDIO_SdmmcCap, virtualSdio2_open, virtualSdio2_writeSectors, virtualSdio2_readSectors, virtualSdio2_format, virtualSdio2_close, virtualSdio2_openMemBus, virtualSdio2_closeMemBus, NULL, NULL, NULL, NULL, NULL};
//static SDMMC_INFO_STRUCT sdInfo;
//#NT#2011/05/04#HH Chuang -begin
//#NT#detect SDHC and SDXC
//static SDIO_MISC_INFORMATION sdio_Misc_Info;
...
ER virtualSdio2_open(void)
{
    ...
    if (pSdioStrg)
    {
        ...
        if ((guiSdioBaseLBA+guiSdioSecCnt) > pSdioStrg->pStrgCap->uiTotalSectors)
        {
            ...
            debug_err(("^R%s: resize virtual storage to %d sectors\r\n", __func__, guiSdioSecCnt));
        }
        else
        {
            gSDIO_SdmmcCap.uiTotalSectors = guiSdioSecCnt;
        }
        debug_err(("^G%s: total sec: %d\r\n", __func__, guiSdioSecCnt));
...
*/
BOOL virtualSdio2_isOpened(void)
{
    return bIsOpened;
}
...
/**
    ...
    @return
        ...
        - @b Else: lock fail
*/
ER virtualSdio2_lock(void)
{
    ER erReturn;

    erReturn = wai_sem(SEMID_VIRTUAL_SDIO2);
...
/**
    ...
    @return
        ...
        - @b Else: unlock fail
*/
ER virtualSdio2_unlock(void)
{

    if(bIsLocked)
    {
        bIsLocked = FALSE;
        return sig_sem(SEMID_VIRTUAL_SDIO2);
    }
...
static ER virtualSdio2_writeSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt)
{
    ...
    // Check boundary
    if ((ulLBAddr + ulSctCnt) > guiSdioSecCnt)
    {
        debug_err(("^R%s: input LBA/SecCnt %d/%d exceed storage sector %d\r\n", ulLBAddr, ulSctCnt, guiSdioSecCnt));
        return E_SYS;
    }
...
static ER virtualSdio2_readSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt)
{
    ...
    if ((ulLBAddr + ulSctCnt) > guiSdioSecCnt)
    {
        debug_err(("^R%s: input LBA/SecCnt %d/%d exceed storage sector %d\r\n", __func__, ulLBAddr, ulSctCnt, guiSdioSecCnt));
        return E_SYS;
    }
...
*/
PSTRG_TAB virtualSdio2_getStorageObject(void)
{
    return &gSDIOObj;
}
...
/**
    ...
    @return none
*/
void virtualSdio2_setFormatParam(INT8* pOemID, INT8* pVolumeLab, BOOL bRandomVolID, UINT32 uiVolID)
{
    UINT32 i, len;
...
/**
    ...
    @return
        - @b E_OK: operation OK
*/
ER virtualSdio2_setMapping(SDIO_ID sdioID, UINT32 uiBaseLBA, UINT32 uiSecCnt)
{
    if (sdioID == SDIO_ID_1)
    {
