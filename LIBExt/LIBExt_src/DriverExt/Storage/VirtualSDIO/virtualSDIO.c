...
//
static ER virtualSdio_openMemBus(void);
static ER virtualSdio_closeMemBus(void);
static ER virtualSdio_writeSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt);
static ER virtualSdio_readSectors(INT8 *pcBuf, UINT32 ulLBAddr, UINT32 ulSctCnt);
static ER virtualSdio_format(void);
...
static STRG_TAB gSDIOObj = {STORAGE_DETACH|STORAGE_SDIO, &gSDIO_SdmmcCap, virtualSdio_open, virtualSdio_writeSectors, virtualSdio_readSectors, virtualSdio_format, virtualSdio_close, virtualSdio_openMemBus, virtualSdio_closeMemBus, NULL, NULL, NULL, NULL, NULL};
...
*/
BOOL virtualSdio_isOpened(void)
{
    return bIsOpened;
}
...
/**
    ...
    @return
        ...
        - @b Else: lock fail
*/
ER virtualSdio_lock(void)
{
    ER erReturn;

    erReturn = wai_sem(SEMID_VIRTUAL_SDIO);
...
/**
    ...
    @return
        ...
        - @b Else: unlock fail
*/
ER virtualSdio_unlock(void)
{

    if(bIsLocked)
    {
        bIsLocked = FALSE;
        return sig_sem(SEMID_VIRTUAL_SDIO);
    }
...
*/
PSTRG_TAB virtualSdio_getStorageObject(void)
{
    return &gSDIOObj;
}
...
/**
    ...
    @return none
*/
void virtualSdio_setFormatParam(INT8* pOemID, INT8* pVolumeLab, BOOL bRandomVolID, UINT32 uiVolID)
{
    UINT32 i, len;
...
/**
    ...
    @return
        - @b E_OK: operation OK
*/
ER virtualSdio_setMapping(SDIO_ID sdioID, UINT32 uiBaseLBA, UINT32 uiSecCnt)
{
    if (sdioID == SDIO_ID_1)
    {
