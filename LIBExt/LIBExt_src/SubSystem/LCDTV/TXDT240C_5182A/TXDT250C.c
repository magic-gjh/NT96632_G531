...
debug_err((">>>>>Init LCD mode %d\r\n",LCD_DeviceObj.uiLCDMode));
    switch (LCD_DeviceObj.uiLCDMode)
    {
        ...
        break;
    case LCD_MODE_RGBD:
        sendtoAUDN01LCDreg(0x00,0x0d);
        TimerDelayMs(10);
        sendtoAUDN01LCDreg(0x00,0x05);
        TimerDelayMs(10);
        sendtoAUDN01LCDreg(0x00,0x05);
        TimerDelayMs(50);
        sendtoAUDN01LCDreg(0x00,0x0f);
        TimerDelayMs(10);
		...
		TimerDelayMs(1);
		sendtoAUDN01LCDreg(0x60,0x01);
        //#PIC#2010/08/10#Creator -end
        sendtoAUDN01LCDreg(0x40,0x00);
        //sendtoAUDN01LCDreg(0x60,0x01);
        sendtoAUDN01LCDreg(0xC0,0x02);
        sendtoAUDN01LCDreg(0x00,0x0F);
        sendtoAUDN01LCDreg(0xf5,0x46);
        sendtoAUDN01LCDreg(0x50,0x00);//0x7c
        sendtoAUDN01LCDreg(0x30,0x0A);
        sendtoAUDN01LCDreg(0x70,0x38); 
        break;
    case LCD_MODE_YUV:
...
//mark for compiling error,        pinmux_select_lcd(PINMUX_LCD_CASIONEW);
    ...
    case LCD_MODE_RGBD:
        ...
        ide_set_dithering(0, 0);
        ide_set_device(DISPLAY_DEVICE_CASIO2G);//DISPLAY_DEVICE_TOPPOLY);
        ide_set_pdir(0);
        ide_set_odd(0);
        ide_set_even(1);
        ide_set_hsinv(1);
