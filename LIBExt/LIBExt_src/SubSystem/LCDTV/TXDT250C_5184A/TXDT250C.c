...
void OpenPanel(LCD_DEVICE_OBJ *lcddeviceobj, LCDSIZE *psizeDisplay)
{
    ...
    if (LCD_DeviceObj.fIDEDstCLK == 0)
    {
        ...
        case LCD_MODE_RGBD:
            LCD_DeviceObj.fIDEDstCLK = 18;//18;//27;
            break;
        //#NT#2007/03/07#Shirley Cheng -begin
        case LCD_MODE_YUV:
            LCD_DeviceObj.fIDEDstCLK = 27;
...
debug_err((">>>>>Init LCD mode %d\r\n",LCD_DeviceObj.uiLCDMode));
    switch (LCD_DeviceObj.uiLCDMode)
    {
        ...
        sendtoAUDN01LCDreg(0x05,0x5D);  //OP1
        break;
    case LCD_MODE_RGBD:
        ...
        TimerDelayMs(10);
        sendtoAUDN01LCDreg(0x2F,0x44);//for TN mode//
           
        //-----VCOMDC setting Control //
        sendtoAUDN01LCDreg(0x9E,0x80);
        sendtoAUDN01LCDreg(0x00,0x02);//VCAC setting//
        sendtoAUDN01LCDreg(0x01,0x9a);//0xa3//0x9B);//(0x01af-20);//VCOMDC setting//
        	//------Gamma Control //
        sendtoAUDN01LCDreg(0xA1,0x01);//Gamma setting //
        sendtoAUDN01LCDreg(0x0d,0x48);//Sleep Out//
        sendtoAUDN01LCDreg(0x03,0x40);//0x48);//Sleep Out//
        
        sendtoAUDN01LCDreg(0x05,0x5e);// //
        
        //-------VCOMH //
        sendtoAUDN01LCDreg(0x80,0x0f); // V63 //
        sendtoAUDN01LCDreg(0x81,0x18); // V60 //
        sendtoAUDN01LCDreg(0x82,0x1c); // V57 //
        sendtoAUDN01LCDreg(0x83,0x0c); // V55 //
        sendtoAUDN01LCDreg(0x84,0x0d); // V52 //
        sendtoAUDN01LCDreg(0x85,0x0b); // V48 //
        sendtoAUDN01LCDreg(0x86,0x0b); // V43 //
        sendtoAUDN01LCDreg(0x87,0x04); // V38 //
        sendtoAUDN01LCDreg(0x88,0x08); // V25 //
        sendtoAUDN01LCDreg(0x89,0x0f); // V20 //
        sendtoAUDN01LCDreg(0x8A,0x12); // V15 //
        sendtoAUDN01LCDreg(0x8B,0x0f); // V11 //
        sendtoAUDN01LCDreg(0x8C,0x0f); // V8 //
        sendtoAUDN01LCDreg(0x8D,0x30); // V6 //
        sendtoAUDN01LCDreg(0x8E,0x27); // V3 //
        sendtoAUDN01LCDreg(0x8F,0x13); // V0 //
        //-------VCOML //
        sendtoAUDN01LCDreg(0x30,0x0e); // V0 //
        sendtoAUDN01LCDreg(0x31,0x25); // V3 //
        sendtoAUDN01LCDreg(0x32,0x30); // V6 //
        sendtoAUDN01LCDreg(0x33,0x0f); // V8 //
        sendtoAUDN01LCDreg(0x34,0x0f); // V11 //
        sendtoAUDN01LCDreg(0x35,0x1d); // V15 //
        sendtoAUDN01LCDreg(0x36,0x0f); // V20 //
        sendtoAUDN01LCDreg(0x37,0x0c); // V25 //
        sendtoAUDN01LCDreg(0x38,0x01); // V38 //
        sendtoAUDN01LCDreg(0x39,0x08); // V43 //
        sendtoAUDN01LCDreg(0x3A,0x00); // V48 //
        sendtoAUDN01LCDreg(0x3B,0x0f); // V52 //
        sendtoAUDN01LCDreg(0x3C,0x08); // V55//
        sendtoAUDN01LCDreg(0x3D,0x28); // V57 //
        sendtoAUDN01LCDreg(0x3E,0x25); // V60 //
        sendtoAUDN01LCDreg(0x3F,0x19); // V63 //
        //sendtoAUDN01LCDreg(0x55,0x40); // V63 //

        sendtoAUDN01LCDreg(0x2B,0x01);//
        
        sendtoAUDN01LCDreg(0x09,0x03);//0x1f);//for TN mode//
        sendtoAUDN01LCDreg(0x0A,0x03);//0x1f);//for TN mode//
        if (GPIOMap_DetLCDRotate())
        {
            sendtoAUDN01LCDreg(0x04,0x1b);//0x1f);//for TN mode//
        }else
        {
...
//mark for compiling error,        pinmux_select_lcd(PINMUX_LCD_CASIONEW);
    ...
    case LCD_MODE_RGBD:
        ...
        ide_set_dithering(0, 0);
        ide_set_device(DISPLAY_DEVICE_CASIO2G);//DISPLAY_DEVICE_CASIO2G);//DISPLAY_DEVICE_TOPPOLY);
        ide_set_pdir(0);
        ide_set_odd(0);
        ide_set_even(1);
        ide_set_hsinv(1);
