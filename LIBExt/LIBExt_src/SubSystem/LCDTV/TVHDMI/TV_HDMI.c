...
static void tvhdmi_configAV(void);
static BOOL tvhdmi_initVidFormat(pVID_FORMAT pVID,    HDMI_VIDEOID VID_Code);
static void tvhdmi_initDisplay(VID_FORMAT VID);
static void tvhdmi_checkClkFreq(void);


// Static variable
static LCD_DEVICE_OBJ   LCD_DeviceObj;

static HDMI_MODE        gHDMI_Mode  =   HDMI_MODE_RGB444;// Useless in current usage.
static HDMI_VIDEOID     gVID        =   HDMI_1280X720P60;
static HDMI_AUDIOFMT    gAID        =   HDMI_AUDIO48KHZ;

static UINT32   guiLoopDiv = HDMI_108_PLL2_LOOPDIV;
static UINT32   guiPerDiv  = HDMI_108_PLL2_PREDIV;
...
/*
    Config HDMI TX's Audio/Video configuration to the specified type.
*/
void tvhdmi_configAV(void)
{
...
static BOOL tvhdmi_initVidFormat(pVID_FORMAT pVID,    HDMI_VIDEOID VID_Code)
{
    ...
    if(Valid == 0)
    {
        debug_err(("Non-supported Video Format!!\r\n"));
        return FALSE;
    }
    ...
    //
            ...
            {
                // Overscan
        ide_setConstantWindowOffset(VideoTiming[idx].WINX_OFS, VideoTiming[idx].WINY_OFS);

        guiBufWinHoriOffset = VideoTiming[idx].WINX_OFS<<1;
        guiBufWinVertOffset = VideoTiming[idx].WINY_OFS<<1;
            }
            else
            {
    ...
    //
    //  Map the video timing details
    //
    pVID->VID                       =   VideoTiming[idx].VID;
    pVID->HS                        =   VideoTiming[idx].HS;
    pVID->VS                        =   VideoTiming[idx].VS;
    pVID->HVLD                      =   VideoTiming[idx].HVLD;
    pVID->VVLD                      =   VideoTiming[idx].VVLD;
    pVID->CLK                       =   VideoTiming[idx].CLK;
    pVID->DE                        =   VideoTiming[idx].DE;
    pVID->PROGRESSIVE               =   VideoTiming[idx].PROGRESSIVE;
    pVID->WIDTH                     =  (VideoTiming[idx].H_ENDTIME     - VideoTiming[idx].H_STARTTIME);
    pVID->HEIGHT                    =  (VideoTiming[idx].V_ODD_ENDTIME - VideoTiming[idx].V_ODD_STARTTIME);
    pVID->H_SYNC_TIME               =   VideoTiming[idx].H_SYNCTIME;
    pVID->H_PERIOD_TIME             =   VideoTiming[idx].H_TOTAL;
    pVID->H_VALID_START_TIME        =   VideoTiming[idx].H_STARTTIME;
    pVID->H_VALID_END_TIME          =   VideoTiming[idx].H_ENDTIME;
    pVID->V_SYNC_TIME               =   VideoTiming[idx].V_SYNCTIME;
    pVID->V_PERIOD_TIME             =   VideoTiming[idx].V_TOTAL;
    pVID->V_ODD_VALID_START_TIME    =   VideoTiming[idx].V_ODD_STARTTIME;
    pVID->V_ODD_VALID_END_TIME      =   VideoTiming[idx].V_ODD_ENDTIME;
    pVID->V_EVEN_VALID_START_TIME   =   VideoTiming[idx].V_EVEN_STARTTIME;
    pVID->V_EVEN_VALID_END_TIME     =   VideoTiming[idx].V_EVEN_ENDTIME;

    if(hdmitx_getEdidInfo()&HDMITX_TV_YCBCR444)
    {
        pVID->DISP_DEV  =   HDMI_MODE_YCBCR444;
    }
    else
    {
        pVID->DISP_DEV  =   HDMI_MODE_RGB444;
    }
...
static void tvhdmi_initDisplay(VID_FORMAT VID)
{
    ...
    if(VID.DISP_DEV==HDMI_MODE_YCBCR422)
    {
        /* 16bits */
    ...
    ide_setHsInv(VID.HS);       /* active high/low */
    ide_setVsInv(VID.VS);       /* active high/low */
    ide_setHvldInv(VID.HVLD);   /* active high/low */
    ide_setVvldInv(VID.VVLD);   /* active high/low */
    ide_setClkInv(VID.CLK);     /* active high/low */
    ide_setDeInv(VID.DE);       /* DE active high/low */

    ide_setFldInv(FALSE);
    ide_setRgbd(FALSE);
    ide_setThrough(FALSE);

    ide_setHorTiming(VID.H_SYNC_TIME-1, VID.H_PERIOD_TIME-1, VID.H_VALID_START_TIME, VID.H_VALID_END_TIME-1);
    ide_setVerTiming(VID.V_SYNC_TIME-1, VID.V_PERIOD_TIME-1, VID.V_ODD_VALID_START_TIME, VID.V_ODD_VALID_END_TIME-1, VID.V_EVEN_VALID_START_TIME, VID.V_EVEN_VALID_END_TIME-1);
    ...
    ide_setSifStartEnd(0x00, 0x00);

    if(VID.DISP_DEV==HDMI_MODE_RGB444)
    {
    ...
    if(VID.DISP_DEV==HDMI_MODE_RGB444) /* output=RGB for 444 mode */
    {
        ide_setOutComp(IDE_COMPONENT_R,IDE_COMPONENT_G,IDE_COMPONENT_B, FALSE, FALSE);
    }
    else if(VID.DISP_DEV==HDMI_MODE_YCBCR444)
    {
    ...
    //ide_setLoad();
    //Wait for DE signal to be more stable
    //ide_waitFrameEnd();
}
...
static void tvhdmi_checkClkFreq(void)
{
    ...
    //
    //  Check any modules except IDE is using PLL2
    //
    uiClkRate = pll_getClockRate(PLL_CLKSEL_CPU);
    if((uiClkRate == PLL_CLKSEL_CPU_PLL2) || (uiClkRate == PLL_CLKSEL_CPU_PLL2_D2))
    {
        debug_err(("OpenTVHDMI change PLL2 procedure may have RISK! (CPU)\r\n"));
    }

    uiClkRate = pll_getClockRate(PLL_CLKSEL_JPEG);
    if((uiClkRate == PLL_CLKSEL_JPEG_PLL2_D2) || (uiClkRate == PLL_CLKSEL_JPEG_PLL2_D3))
    {
        debug_err(("OpenTVHDMI change PLL2 procedure may have RISK! (JPEG)\r\n"));
    }

    uiClkRate = pll_getClockRate(PLL_CLKSEL_H264);
    if((uiClkRate == PLL_CLKSEL_H264_PLL2_D3) || (uiClkRate == PLL_CLKSEL_H264_PLL2_D4))
    {
        debug_err(("OpenTVHDMI change PLL2 procedure may have RISK! (H264)\r\n"));
    }

    uiClkRate = pll_getClockRate(PLL_CLKSEL_GRAPHIC);
    if((uiClkRate == PLL_CLKSEL_GRAPHIC_PLL2) || (uiClkRate == PLL_CLKSEL_GRAPHIC_PLL2_D2))
    {
        debug_err(("OpenTVHDMI change PLL2 procedure may have RISK! (GRAPHIC)\r\n"));
    }

    uiClkRate = pll_getClockRate(PLL_CLKSEL_SIE_MCLKSRC);
    if(uiClkRate == PLL_CLKSEL_SIE_MCLKSRC_PLL2)
    {
        debug_err(("OpenTVHDMI change PLL2 procedure may have RISK! (SIE-MCLK)\r\n"));
    }
...
BOOL tvhdmi_changeClkFreq(BOOL bNative)
{
    ...
    switch(gVID)
    {
        ...
        case HDMI_720X480I120_16X9:     /* VID51 */
            if((LoopDiv != HDMI_108_PLL2_LOOPDIV) || (PreDiv != HDMI_108_PLL2_PREDIV) )
            {
                pll_setPLL2En(FALSE);
                pll_setPLL2(HDMI_108_PLL2_LOOPDIV, HDMI_108_PLL2_PREDIV);
                pll_setPLL2En(TRUE);
            }
            break;

        /* Change PLL2 to 144Mhz */
        case HDMI_1920X1080I50_VT1250:  /* VID39 */
            if((LoopDiv != HDMI_144_PLL2_LOOPDIV) || (PreDiv != HDMI_144_PLL2_PREDIV) )
            {
                pll_setPLL2En(FALSE);
                pll_setPLL2(HDMI_144_PLL2_LOOPDIV, HDMI_144_PLL2_PREDIV);
                pll_setPLL2En(TRUE);
            }
            break;
        ...
        case HDMI_720X480I240_16X9:     /* VID59 */
            if((LoopDiv != HDMI_216_PLL2_LOOPDIV) || (PreDiv != HDMI_216_PLL2_PREDIV) )
            {
                pll_setPLL2En(FALSE);
                pll_setPLL2(HDMI_216_PLL2_LOOPDIV, HDMI_216_PLL2_PREDIV);
                pll_setPLL2En(TRUE);
            }
            break;
        ...
        case HDMI_640X480P60:           /* VID1 */
            if((LoopDiv != HDMI_252_PLL2_LOOPDIV) || (PreDiv != HDMI_252_PLL2_PREDIV) )
            {
                pll_setPLL2En(FALSE);
                pll_setPLL2(HDMI_252_PLL2_LOOPDIV, HDMI_252_PLL2_PREDIV);
                pll_setPLL2En(TRUE);
            }
            break;
        ...
        case HDMI_1280X720P120:         /* VID47 */
            if((LoopDiv != HDMI_297_PLL2_LOOPDIV) || (PreDiv != HDMI_297_PLL2_PREDIV) )
            {
                pll_setPLL2En(FALSE);
                pll_setPLL2(HDMI_297_PLL2_LOOPDIV, HDMI_297_PLL2_PREDIV);
                pll_setPLL2En(TRUE);
            }
            break;

        default:
            debug_err(("Not support HDMI resolution mode\r\n"));
            return TRUE;
    }
    ...
    switch(gVID)
    {
        ...
        case HDMI_1920X1080I50_VT1250:
            LCD_DeviceObj.fIDEDstCLK = 72;
            uiClockDiv = (((float)LCD_DeviceObj.uiIDESrcCLK/2.0)+71.99)/LCD_DeviceObj.fIDEDstCLK;
            break;
...
void OpenTVHDMI(LCD_DEVICE_OBJ *lcddeviceobj, LCDSIZE *psizeDisplay)
{
    ...
    if(hdmitx_isOpened()==FALSE)
    {
        if(ide_getEn()==TRUE)
        {
            ...
            ide_set_en(FALSE);
            ide_waitFrameEnd();
            debug_err(("IDE should be already Disabled!! \r\n"));
        }
        ...
        // 630 HDMI is internal HW engine, close pins out.
    ...
    if(tvhdmi_initVidFormat(&VID,gVID)==FALSE)
        return;

    // Configure IDE
    tvhdmi_initDisplay(VID);

    psizeDisplay->uiWidth       = VID.WIDTH;//-guiBufWinHoriOffset;
    psizeDisplay->uiHeight      = VID.HEIGHT;//-guiBufWinVertOffset;
    psizeDisplay->uiWinWidth    = VID.WIDTH-guiBufWinHoriOffset;
    psizeDisplay->uiWinHeight   = VID.HEIGHT-guiBufWinVertOffset;
    ...
    else
    {
        ...
        psizeDisplay->uiWidth       = VID.WIDTH;
        psizeDisplay->uiHeight      = VID.HEIGHT;
        psizeDisplay->uiWinWidth    = VID.WIDTH-guiBufWinHoriOffset;
        psizeDisplay->uiWinHeight   = VID.HEIGHT-guiBufWinVertOffset;
