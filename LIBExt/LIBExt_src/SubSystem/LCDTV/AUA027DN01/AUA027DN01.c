...
//#NT#2009/11/24#SA4 -begin
#define INPUT_FORMAT_UPS052_320                   0x18//0x2B
#define INPUT_FORMAT_UPS052_320_ROTATE            0x1b//0x28
//#NT#2009/11/24#SA4 -begin
...
void OpenPanel(LCD_DEVICE_OBJ *lcddeviceobj, LCDSIZE *psizeDisplay)
{
    ...
    debug_err(("Open OpenAUDN01!!!!!!!!!!!!!\n\r"));
    OpenAUDN01(psizeDisplay);
    debug_err(("Open OpenAUDN01 Fin!!!!!!!!!!!!!\n\r"));
}
...
void readtoAUDN01LCDreg_gpio(UINT8 addr, UINT8 *value2)
{
    UINT32 data = 0;
    UINT32 j;
    UINT8 value = 0;
    ...
    data = (UINT32)(addr|0x40) << 8 | (UINT32)value;
    //data = ((UINT32)addr << 10) | (0x1 << 9) | (UINT32)value;
    debug_err(("[R]data=0x%x,addr = %d\n\r",data,addr));
    ...
    for(j = 16; j > 0; j--)
    {
        ...
        {
            //debug_err(("j=%d data=%d\n\r",j,gpio_getPin(LCD_DeviceObj.LCDCtrl_Pin.uigpio_data)));
        }
        delay(150);
        gpio_clearPin(LCD_DeviceObj.LCDCtrl_Pin.uigpio_clk);
        delay(200);
        ...
        {
            debug_err(("j=%d data=%d\n\r",j,gpio_getPin(LCD_DeviceObj.LCDCtrl_Pin.uigpio_data)));
            value = value | (gpio_getPin(LCD_DeviceObj.LCDCtrl_Pin.uigpio_data)<<(j-1));
        }
        delay(150);
    }
    ...
    gpio_setPin(LCD_DeviceObj.LCDCtrl_Pin.uigpio_sen);
    *value2 = value;
    debug_err(("addr=0x%x,value=0x%0x\r\n",addr,value));
}
...
static void Init_AUDN01_LCD(void)
{
    ...
    case LCD_MODE_RGBD:
        #if 0
        sendtoAUDN01LCDreg(0x00,0x00/*0x03*/);//sendtoAUBL02LCDreg(0x00,0x0F);
        //sendtoAUDN01LCDreg(0x40,0x03);
        //while(1)
        {
            sendtoAUDN01LCDreg(0x04,INPUT_FORMAT_UPS052_320);  //UPS052 360x240 --> 2B
            //TimerDelayMs(1000);
        }
        ...
        sendtoAUDN01LCDreg(0x0b,0x81);  //67
        sendtoAUDN01LCDreg(0x01,0x95);  //67
        sendtoAUDN01LCDreg(0x00,0x07);
        sendtoAUDN01LCDreg(0x05,0x5d);
        //sendtoAUDN01LCDreg(0x06,0x03);
        //sendtoAUDN01LCDreg(0x07,0x32);

        sendtoAUDN01LCDreg(0x16,0x00);
        sendtoAUDN01LCDreg(0x17,0x00);
        sendtoAUDN01LCDreg(0x18,0x55);
        sendtoAUDN01LCDreg(0x19,0x77);
        sendtoAUDN01LCDreg(0x1a,0x77);
        sendtoAUDN01LCDreg(0x04,0x18);  //UPS052 360x240 --> 2B

        //sendtoAUDN01LCDreg(0x04,0x0f);
        sendtoAUDN01LCDreg(0x97,0x03);
        sendtoAUDN01LCDreg(0x96,0x02);
        sendtoAUDN01LCDreg(0xac,0x90);
        #endif
        //#NT#2009/12/23#Randy -begin
        //fix panel¹d¾¦ issue
        sendtoAUDN01LCDreg(0xb3,0x01);
        sendtoAUDN01LCDreg(0xb5,0x06);
        //#NT#2009/12/23#Randy -end
...
void OpenAUDN01(LCDSIZE *psizeDisplay)
{
    ...
    case LCD_MODE_RGBD:
        ...
        ide_set_ver_timing(0x01, 0x106, 0x15+1, 0x105+1, 0x15+1, 0x105+1);
...
void LCD_Rotate(UINT32 bRotate)
{
    ...
    if (bRotate)
  {
        ...
        ide_set_load();
        sendtoAUDN01LCDreg(0x04,INPUT_FORMAT_UPS052_320_ROTATE);
    }				    
    else
    {
        ide_wait_frame_end();
        ...
        ide_set_pdir(IDE_PDIR_RGB);
        ide_set_load();
        sendtoAUDN01LCDreg(0x04,INPUT_FORMAT_UPS052_320);
    }
    return;
}
