...
//#NT#Since nand API is removed from file system, the following function should be modified in future.
#if 0
    tMSDCVENDOR_CTRL* pCtrl = MsdcVendorNvt_GetCtrl();
    tMSDCVENDOR_HOST_INFO* pHost = &pCtrl->tHostInfo;
    ...
    pBlkInfo->nBytesPerBlock = FilesysNANDGetBlockSize();
...
//#NT#Since nand API is removed from file system, the following function should be modified in future.
    ...
    INT BlkSize = FilesysNANDGetBlockSize();
    ...
    pNand->Open();
    CPUCleanDCacheBlock((UINT32)pData,(UINT32)(pData+BlkSize));
