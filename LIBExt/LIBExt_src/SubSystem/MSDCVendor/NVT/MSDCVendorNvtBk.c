...
{
    const char fName[] = "MsdcVendorNvt_Bk_HostLock():";
    tMSDCVENDOR_BACKGROUND_CTRL* pBk = &MsdcVendorNvt_GetCtrl()->tBkCtrl;
    
    if(pBk->bServiceLock)
    {
        debug_err(("^R%s already locked!\r\n",fName));
        return FALSE;
    }
...
{
    const char fName[] = "MsdcVendorNvt_Bk_HostUnLock():";
    tMSDCVENDOR_BACKGROUND_CTRL* pBk = &MsdcVendorNvt_GetCtrl()->tBkCtrl;

    if(!pBk->bServiceLock)
    {
        debug_err(("^R%s not locked!\r\n",fName));
        return FALSE;
    }
    else if(pBk->bCmdRunning)
    {
        debug_err(("^R%s Bk is Running\r\n",fName));
        return FALSE;
    }
