        ...
        if ( GammaFrmCnt == 0 )
        {
            if ( gDynaGammaSetting == TRUE )
            {
                if ( LastLV < 0 )
                    ...
                    CurLV = (LastLV*7 + AE__GetLV())>>3;
                LastLV = CurLV;
                temp = IQ_GammaLUT[1];
                for (i=0; i< 130; i++ )
                    IQ_GammaLUT[i] = InterpolateValue(CurLV, GammaLUT_128Tab_Video_Night[i], GammaLUT_128Tab[i], 40, 80);
                if ( IQ_GammaLUT[1] != temp )
                    CPU_WriteGammaTab(IQ_GammaLUT, IQ_GammaLUT, IQ_GammaLUT);
                //debug_err(("%d %d %d   \r\n", CurLV, AE__GetLV(), IQ_GammaLUT[1]));
            }
            if ( gDynaGammaSetting == FALSE )
            {
                CPU_WriteGammaTab(GammaLUT_128Tab, GammaLUT_128Tab, GammaLUT_128Tab);
            }
        }
        GammaFrmCnt+=1;
        if ( GammaFrmCnt> 3 ) GammaFrmCnt = 0;
