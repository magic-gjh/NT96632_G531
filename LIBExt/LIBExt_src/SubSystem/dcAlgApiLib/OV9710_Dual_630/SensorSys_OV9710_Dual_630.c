...
UINT32  SensorModeMap(_IMGResolution_sel _mode)
{
    switch(_mode)
    {
        ...
        case _Resolution_1280x960:
            UI_SetParameter(_ImageRatio, _Ratio_4_3);
            return MONITOR_MODE;
//#NT#2011/08/22#Jarkko Chang -begin
//#NT# for dual sensor ratio 16:5(1280x400) and 8:3(1920x720)
        case _Resolution_1280x400:
            UI_SetParameter(_ImageRatio, _Ratio_16_5);
            return MONITOR_MODE_4;
        case _Resolution_1920x720:
        case _Resolution_1280x480:
            UI_SetParameter(_ImageRatio, _Ratio_8_3);
            return MONITOR_MODE_4;
//#NT#2011/08/22#Jarkko Chang -end
        default:
            UI_SetParameter(_ImageRatio, _Ratio_4_3);
            return MONITOR_MODE;
        break;
...
void Set2Preview(IPL_OBJ *Obj)
{
    ...
    SIE_PRE_Para_prv.PreSrcSel = PATH_POST_GAMMA;//PATH_PRE_CLOLRGAIN;
//    SIE_PRE_Para_prv.PreFuncEn = DISABLE;
    ...
    {
        debug_err(("_Prv_B_Status=(%d)%d ,_Prv_W_Status=(%d)%d\r\n",_Prv_B_Status,CalInfo->CalStatus_PrvDP_B,_Prv_W_Status,CalInfo->CalStatus_PrvDP_W));
    }
    ...
    if(g_fpAlgExport)
    {
        if(Get_IrLedIndex())
        {
            Ae_FastConverge2TargetLumi(GammaLUT_128Tab_Video_Night,_SieBitDepth_10,0);
        }
		#if 1
        else
        {
            Ae_FastConverge2TargetLumi(GammaLUT_128Tab_Video_Normal,_SieBitDepth_10,0);
        }
		#endif
...
//#NT#2009/08/25#Jarkko Chang -end
  ...
  if(job_req->SIE_PRE_PRV->PreFuncEn & PRE_VIG_EN)
  {
        ...
        else if(job_req->SIE_PRE_PRV->ModeSel == MONITOR_MODE_4)
        {
            job_req->pre_vig_setting.VIG_R_LUT = Shading_Table720p;
            job_req->pre_vig_setting.VIG_G_LUT = Shading_Table720p;
            job_req->pre_vig_setting.VIG_B_LUT = Shading_Table720p;
  ...
  if(job_req->SIE_PRE_PRV->PreFuncEn & PRE_GEO_EN){
    ...
    if(job_req->SIE_PRE_PRV->PreFuncEn & PRE_GAMMA_EN)
    {
            ...
            if(Get_IrLedIndex())
            {
                job_req->pre_gamma_r = GammaLUT_128Tab_Video_Night;
                job_req->pre_gamma_g = GammaLUT_128Tab_Video_Night;
                job_req->pre_gamma_b = GammaLUT_128Tab_Video_Night;
            }
			#if 1
            else
            {
                job_req->pre_gamma_r = GammaLUT_128Tab_Video_Normal;
                job_req->pre_gamma_g = GammaLUT_128Tab_Video_Normal;
                job_req->pre_gamma_b = GammaLUT_128Tab_Video_Normal;
            }
			#endif
    }
...
void pre_CapParameter(struct  PUBLIC_STR_JOBREQS *job_req)
{
  ...
  if(job_req->SIE_PRE_CAP->PreFuncEn & PRE_DF_SUB_EN){
    ...
    if(job_req->SIE_PRE_CAP->PreFuncEn & PRE_GAMMA_EN)
    {
        gDynaGammaSetting = FALSE;
        job_req->pre_gamma_r = GammaLUT_128Tab_Capture;
        job_req->pre_gamma_g = GammaLUT_128Tab_Capture;
        job_req->pre_gamma_b = GammaLUT_128Tab_Capture;
    }
}
...
//            pll_setClockRate(PLL_CLKSEL_SIE_MCLKSRC, PLL_CLKSEL_SIE_MCLKSRC_480);
            ...
            debug_err(("^R_Vid_ResolutionIdx = %d\r\n", GetIPPSizeInfor(_Vid_ResolutionIdx)));
            ...
            {
                pll_setClockFreq(SIEMCLK_FREQ, 8000000);
            }
            else if (GetIPPSizeInfor(_Vid_ResolutionIdx) == _Resolution_1920x720)
            {
                pll_setClockFreq(SIEMCLK_FREQ, 12000000); //20 fps
            }
            else
            {
                pll_setClockFreq(SIEMCLK_FREQ, 24000000);
...
//            SetIPPSizeInfor(_SieXcenter, SIE_CAP_XCENTER_1);  //using preview setting
//            SetIPPSizeInfor(_SieYcenter, SIE_CAP_YCENTER_1);  //using preview setting
            SetIPPSizeInfor(_HDTotal_Captrue, GetIPPSizeInfor(_HDTotal_Preview));
            SetIPPSizeInfor(_VDTotal_Captrue, GetIPPSizeInfor(_VDTotal_Preview));
...
void IPPFunc_OnOffSwitch(void)
{
    ...
    SetIPPFuncInfor(CAPGAMMACURVE,(UINT32)GammaLUT_128Tab_Capture);
...
//    SiePreInfo->PreFuncEn = DISABLE;
    SiePreInfo->PreFuncEn = PRE_CG_EN|PRE_GAMMA_EN;
