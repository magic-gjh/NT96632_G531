...
SenCBInfo gSenCBInfo = {0, &IPPMgrReq, &SIE_PRE_Para_cap, &SIE_PRE_Para_prv, &IPEModeInfo, SEN_Flip_OFF};
...
#define EEXT_CHANGE_TH_H    (EV_Value_MIN*3)
#define EEXT_CHANGE_TH_L    EV_Value_MIN
...
//#NT#20091123#ethanlau -begin
    ...
    if(EEXT_Type==0x00)
    {
        EEXT_Type = (AE_Param.EV_Value <= EEXT_CHANGE_TH_L) ? 0x01 : 0x00;
    }
    else
    {
        EEXT_Type = (AE_Param.EV_Value >= EEXT_CHANGE_TH_H) ? 0x00 : 0x01;
    }    
