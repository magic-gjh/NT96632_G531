...
#define AWB_STABLE_CNT  2
UINT gAWB_WaitStableCnt=0;
...
void aaa_AWBprocess(void)
{
    ...
    //#NT#For more stable awb process, the awb active will be disabled serveal times after leaving zoom-in.
    gAWB_WaitStableCnt++;
    if(gAWB_WaitStableCnt < AWB_STABLE_CNT)
        return;
    else
        gAWB_WaitStableCnt = AWB_STABLE_CNT;
    //#NT#20120820#clifflin -end
    gAWB_counter++;
    aaa_CAprocess();
    AWB_LV=AE__GetLV();
...
//#NT#2009/08/31#Jarkko Chang -end
    ...
    if(AWB_LV < 60)
    {
        //Sensor_WriteReg(0x3041, 0x00);
        //Sensor_WriteReg(0x3054, 104);
        return;
    }
/*
    Sensor_WriteReg(0x3041, 0x02);
    Sensor_WriteReg(0x3054, 0x00);
*/
    SensorInfor.FP_ClearOSD();

    sprintf(message,"%02X, %02X, %02X, %02X, %02X",
            Sensor_ReadReg(0x301D),
            Sensor_ReadReg(0x305B),
            Sensor_ReadReg(0x305C),
            Sensor_ReadReg(0x305D),
            Sensor_ReadReg(0x305E));
    ...
    if(W_total > 5)
    {
        ...
        {
#if 0
            Rgain = (NewRgain+OldRgain)>>1;
            Bgain = (NewBgain+OldBgain)>>1;
            Ggain = NewGgain;
#else
...
//#NT#2009/08/31#Jarkko Chang -end
    ...
    if(AWB_DebugMsg)
    {
        SenFP_ClearOSD();
        //sprintf(message,"LV:%d",AWB_LV);
        sprintf(message,"%d, %d, %d, %d", AWB_LV, Rgain, Ggain, Bgain);
        SenFP_ShowOSDString(message,20,100,4);
/*            
        if((gAWB_counter%5)==0)
        {
...
UINT GetSaturation(UINT RR, UINT GG, UINT BB)
{
    UINT Max,Min;
    ...
    //$$
    if((Max+Min)<1)
       return(100*((UINT)Max-(UINT)Min)/(1+(UINT)Max+(UINT)Min));
    return(100*((UINT)Max-(UINT)Min)/((UINT)Max+(UINT)Min));
}
...
UINT AwbCheckWhite(UINT R,UINT G,UINT B,environ _environ)
{
    ...
    _Sat = GetSaturation(R, G, B);
        ...
        if(rgain<Thr_GR_L2)
            ...
            return GainMux_min;
        else if(_Sat > Thr_Sat_H2)
            return Sat_max;
        else if(_Sat < Thr_Sat_L2)
            return Sat_min;
#else
    if(_environ==_indoor)
    {
