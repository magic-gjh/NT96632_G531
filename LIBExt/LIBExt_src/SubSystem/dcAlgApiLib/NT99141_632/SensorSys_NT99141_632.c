...
#define SENSORLIB_VERSION   "NT99141_2013/02/05_V1"
...
//#NT#For more stable awb process, the awb active will be disabled serveal times after leaving zoom-in.
extern UINT gAWB_WaitStableCnt;
...
//    SIE_PRE_Para_prv.PreFuncEn = DISABLE;
    ...
    {
        SetIPPSizeInfor(_OBOffset, 0x00);
    }
...
//add for init in video mode
    ...
    //#NT#Modify for mi1030
    SIE_PRE_Para_prv.SieDataPhase = 1;
...
//#NT#2009/08/25#Jarkko Chang -end
  ...
  if(job_req->SIE_PRE_PRV->PreFuncEn & PRE_VIG_EN)
  {
    ...
    //#NT#2009/08/04#Jarkko Chang -end
          ...
          job_req->pre_vig_setting.VIG_XR = (IMAGE_SIZE_H>>1);
          job_req->pre_vig_setting.VIG_YR = (IMAGE_SIZE_V>>1);
          job_req->pre_vig_setting.VIG_XGR = (IMAGE_SIZE_H>>1);
          job_req->pre_vig_setting.VIG_YGR = (IMAGE_SIZE_V>>1);
          job_req->pre_vig_setting.VIG_XGB = (IMAGE_SIZE_H>>1);
          job_req->pre_vig_setting.VIG_YGB = (IMAGE_SIZE_V>>1);
          job_req->pre_vig_setting.VIG_XB = (IMAGE_SIZE_H>>1);
          job_req->pre_vig_setting.VIG_YB = (IMAGE_SIZE_V>>1);
          job_req->pre_vig_setting.VIG_YDIST = 0;        
          job_req->pre_vig_setting.VIG_MUL = 103;
          job_req->pre_vig_setting.VIG_T = 4;
...
void SetSharpness(unsigned char Index)
{
    ...
    IQInfo.EDGE_PARAM->EdgEnhance = ((IQInfo.EDGE_PARAM->EdgEnhance*Sharpness[GetISOIdx(AE_Param.ISO_Gain)][Index])/100);
...
unsigned int WriteDebugData(unsigned char *Add)
{
    ...
    buff += sprintf((char *)buff,SENSORLIB_VERSION);
    buff += sprintf((char *)buff,"%s", endl);
...
void SetSiePreCapParam(SIE_PRE_PARAM *SiePreInfo)
{
    ...
    if(0)// (SiePreInfo->AutoObEn == DISABLE)
    {
