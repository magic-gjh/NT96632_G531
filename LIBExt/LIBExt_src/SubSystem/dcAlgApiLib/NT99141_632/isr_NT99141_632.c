...
#define DYNAMIC_GAMMA_YAVG_TH_L         50
#define DYNAMIC_GAMMA_YAVG_TH_H         110
#define DYNAMIC_GAMMA_YAVG_TH_DIFF      (DYNAMIC_GAMMA_YAVG_TH_H-DYNAMIC_GAMMA_YAVG_TH_L)

#define DYNAMIC_OB_VAL_L        0x00
#define DYNAMIC_OB_VAL_H        0x18
#define DYNAMIC_OB_VAL_DIFF     (DYNAMIC_OB_VAL_H-DYNAMIC_OB_VAL_L)

extern UINT32 gDynaGammaSetting;
extern UINT16 gDynaGammaOutL, gDynaGammaOutH;
UINT32 IQ_GammaLUT[130];

extern UINT32 GetInterpolationValue(UINT32 IQ_Value_L, UINT32 IQ_Value_H, UINT32 Ref_Diff, UINT32 Range_RefDiff);
...
UINT Dynamic_OB_Value;
UINT32 PreYavg = 0x00;
extern BOOL bDynaGammaTrigger;

void pre_isrCB(UINT32 IntStatus)
{
    int i;
    UINT32 ValueH, ValueL, Y_Diff;
    ...
    if (IntStatus & PRE_INT_FLDEND)
    {
        ...
        if(gDynaGammaSetting == TRUE )
        {
			...
			for (i=0; i<130; i++)
			{
				IQ_GammaLUT[i] = gDynaGammaOutL+(GammaLUT_128Tab_Video_Normal[i]*(gDynaGammaOutH-gDynaGammaOutL)/1023);
			}
...
#if 0
            Y_Diff = (YAverage > PreYavg) ? (YAverage - PreYavg) : (PreYavg - YAverage);

            if(bDynaGammaTrigger && (Y_Diff > 3))
            {
                bDynaGammaTrigger = FALSE;
                    
                if(YAverage < DYNAMIC_GAMMA_YAVG_TH_L)
                    Y_Diff = DYNAMIC_GAMMA_YAVG_TH_DIFF;
                else if(YAverage > DYNAMIC_GAMMA_YAVG_TH_H)
                    Y_Diff = 0;
                else
                    Y_Diff = DYNAMIC_GAMMA_YAVG_TH_H - YAverage;
                
                for (i=0; i< 130; i++ )
                {
                    ValueH = (GammaLUT_128Tab_Video_Night[i] > GammaLUT_128Tab_Video_Normal[i]) ? GammaLUT_128Tab_Video_Night[i] : GammaLUT_128Tab_Video_Normal[i];
                    ValueL = (GammaLUT_128Tab_Video_Night[i] > GammaLUT_128Tab_Video_Normal[i]) ? GammaLUT_128Tab_Video_Normal[i] : GammaLUT_128Tab_Video_Night[i];
                    ...
                    IQ_GammaLUT[i] = GetInterpolationValue(ValueL, ValueH, Y_Diff, DYNAMIC_GAMMA_YAVG_TH_DIFF);
                }

                Dynamic_OB_Value = GetInterpolationValue(DYNAMIC_OB_VAL_L, DYNAMIC_OB_VAL_H, Y_Diff, DYNAMIC_GAMMA_YAVG_TH_DIFF);
                
                SetIPPSizeInfor(_OBOffset, Dynamic_OB_Value);
                CPU_WriteGammaTab(IQ_GammaLUT, IQ_GammaLUT, IQ_GammaLUT);
            }
