...
BOOL    EnbCustomWB = FALSE;
UINT    gAWB_Custom[2] = {331, 286};

void AWB_SetCustomWB(BOOL Enb)
{
    if (EnbCustomWB && !Enb)
    {
        gAWB_Custom[0] = Rgain;
...
//#NT#modify for ca process
    ...
    CA_para.DramAddr += Sen_GetIPEAddrOfs(gImageAlgInfo.ImageFlip, GetIPPSizeInfor(_PreOutHsize), SIE_PRE_Para_prv.PreOutLineOfs, gImageAlgInfo.PrePrvBitDepth);
    ...
    //#NT#2010/12/30#JJ Huang -begin
    CA_para.Cfapat = 1;//For HJT//The CfaPat translation doesn't fit the raw//Sen_GetIPECfaPat(CFAPAT_PRV, gImageAlgInfo.ImageFlip);
    //#NT#2010/12/30#JJ Huang -end
