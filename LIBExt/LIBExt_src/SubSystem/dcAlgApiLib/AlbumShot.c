...
#define IMG_CAP_FMT         IME_OUT_FMT_422_COSITED
...
void ASGetBufRatio(UINT32 *Numerator, UINT32 *Denominator)
{
    ...
    *Numerator = 15;
    *Denominator = 10;
#elif (IMG_CAP_FMT == IME_OUT_FMT_422_COSITED)
    *Numerator = 20;
    *Denominator = 10;
#endif
}
...
void ASImgProcess(UINT32 Idx)
{
    ...
    fmInfo.ImeHofst2 = fmInfo.ImeHsize2;
    fmInfo.ImeFmt2 = IME_OUT_FMT_422_COSITED;
...
void ASFormJpgFile(UINT32 *JpgAddr, UINT32 *JpgSize)
{
    ...
    //#NT#2010/02/24#Meg Lin -begin
    //#NT#add lineoffset
    JPGParam.LineOffsetY = AlbumInfo.CapSizeH;
    JPGParam.LineOffsetCbCr = AlbumInfo.CapSizeH/2;
    //#NT#2010/02/24#Meg Lin -end
    ...
    EXIFParamB.RawImageAddr[0]  = ippbuf_GetBuf(IPPBUF_CAP_QV_DISPLAY);
    EXIFParamB.RawImageAddr[1] = EXIFParamB.RawImageAddr[0] + (GetIPPSizeInfor(_QVHsize) * GetIPPSizeInfor(_QVVsize));
    EXIFParamB.RawImageAddr[2] = EXIFParamB.RawImageAddr[0] + (GetIPPSizeInfor(_QVHsize) * GetIPPSizeInfor(_QVVsize) * 3 / 2);
    EXIFParamB.JPGBSAddr = ippbuf_GetBuf(IPPBUF_CAP_QV_PRE);
    EXIFParamB.JPGBSBufSize = &ThumbImgSizeB;
    ...
    EXIFParamB.LineOffsetY = EXIFParamB.Width;
    EXIFParamB.LineOffsetCbCr = EXIFParamB.Width/2;
    retry = 0;
    while(retry < 16)
    {

        ThumbImgSizeB = (40 * 1024); //40K
        if (E_OK == JpegEncOnePic(&EXIFParamB, (80 - (retry * 5))))
        {
            break;
        }
        retry ++;
    ...
    //#NT#2010/02/24#Meg Lin -begin
    //#NT#add lineoffset
    JpegCopyEXIFHeader2(&MainImgParam, &EXIFParam, (PEXIFEncParam)&EXIFParamB);
    //#NT#2010/02/24#Meg Lin -end
