...
/**
    ...
    Only for flow control by GPIO simulation.
*/
typedef struct
{
...
/**
    ...
    PIO: Only use in buffer mode.
    DMA: Only use in buffer mode.

    NOTE:Only supports PIO for now, DMA doesn't ready yet.
*/
typedef enum
{
    UART_DATA_TRANS_PIO,                ///< PIO mode
    UART_DATA_TRANS_DMA,                ///< DMA mode
